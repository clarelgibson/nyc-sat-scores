...
# Read in all files and append to the data list of df
data <- c(data, setNames(lapply(survey_files, read_tsv), survey_names))
```

There are actually far too many columns in the survey data to be useful for our analysis. We can resolve this issue by looking at the [data dictionary][7] file that accompanies this data set on the [NYC Open Data][6] site. We'll use the list of fields in the data dictionary for our analysis.
